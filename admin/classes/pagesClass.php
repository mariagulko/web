...
class pagesClass {
  ...
  public function updatePages($page) {
    ...
    $queryUpdate = "UPDATE {$this->table} SET {$url} {$name} {$content} WHERE id={$page['id']}";
  ...
  public function insertPages($page) {
    ...
    if ($first_field == "")
      return 'нет данных для обновления записи';
    $queryInsert = "INSERT INTO {$this->table} (`id`, `url`, `name`, `content`) VALUES (NULL, " . $url . "," . $name . "," . $content . ")";
